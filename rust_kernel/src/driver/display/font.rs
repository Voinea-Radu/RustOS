...
impl Font {
    ...
    pub fn render(&self, x: usize, y: usize, char: char) {
        self.render_asset(x, y, 0, char as usize - ASCII_TABLE_START)
    }
}
...
impl AssetAtlas for Font {
    ...
    fn render_box(
        ...
        self.image
            .render_box(x, y, box_x, box_y, box_width, box_height)
    }
}
...
impl Cursor {
    ...
    pub fn render(&mut self, char: char) {
        if char == '\n' {
            self.render_new_line();
            return;
        }

        self.font.render(self.x * self.font.width, self.y * self.font.height, char);
